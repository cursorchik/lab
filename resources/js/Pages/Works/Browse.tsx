	...
	return (
		<TableBrowse urls={{add: '/works/create'}} title="Список работ">
					...
					<div className={styles.filters} onClick={handleModalClick}>
						...
						<div className={styles.filters_content}>
							...
							<div>
								...
								<textarea id="patient" className="form-control" name="patient"
										  defaultValue={props.filters.patient ?? ''}/>
							</div>
							<div>
								<label htmlFor="mid" className="form-label">Техник</label>
								<select id="mid" className="form-select" name="mid"
							...
							<div>
								...
								<select id="cid" className="form-select" name="cid"
							...
							<div>
								...
								<select id="lock_type" className="form-select" name="lock_type"
				...
				<Modal close={closeUnlockModal}>
					<div className={styles.filters} onClick={handleModalClick}>
							...
							<div className="mt-2">
								{unlockModal.clinicLocked && (
									<div className="form-check">
										<input id="unlock_clinic" className="form-check-input" type="radio"
											   value="clinic" checked={unlockType === 'clinic'}
											   onChange={() => setUnlockType('clinic')}/>
										<label className="form-check-label" htmlFor="unlock_clinic">Снять блокировку
											клиникой</label>
									</div>
								)}
								{unlockModal.mechanicLocked && (
									<div className="form-check">
										<input id="unlock_mechanic" className="form-check-input" type="radio"
											   value="mechanic" checked={unlockType === 'mechanic'}
											   onChange={() => setUnlockType('mechanic')}/>
										<label className="form-check-label" htmlFor="unlock_mechanic">Снять блокировку
											техником</label>
									</div>
								)}
								{(unlockModal.clinicLocked && unlockModal.mechanicLocked) && (
									<div className="form-check">
										<input id="unlock_both" className="form-check-input" type="radio" value="both"
											   checked={unlockType === 'both'} onChange={() => setUnlockType('both')}/>
										<label className="form-check-label" htmlFor="unlock_both">Снять обе
											блокировки</label>
									</div>
								)}
							</div>
						...
						<div className="flex gap-1.5 justify-end">
							<button className="btn btn-outline-dark" onClick={closeUnlockModal}>Отмена</button>
							<button className="btn btn-primary" onClick={handleUnlock}>Снять</button>
						</div>
					</div>
				</Modal>
			)}
			...
			<table className="table text-sm">
				...
				<tr>
					<th className="col">
						<Link href={getSortLink('id')} className="whitespace-nowrap text-decoration-none"
					...
					<tr key={item.id}>
						...
						<td>
							<span className="flex gap-1 align-items-center justify-center">
								...
								{(item.locked_by_clinic || item.locked_by_mechanic) && (
									<a title="Снять блокировку" className="i unlock"
