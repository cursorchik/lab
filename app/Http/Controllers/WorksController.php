...
use App\Models\
{Clinic, ClinicWorkLock, Mechanic, MechanicWorkLock, Work, WorkType};
...
class WorksController extends Controller implements IWork
{
	...
	{
		return ['mechanics' => ['selected' => 0, 'items' => [],], 'clinics' => ['selected' => 0, 'items' => [],], 'date' => date('Y-m-d'), 'patient' => '', 'lock_type' => 'none',];
	}
	...
	public function index(Request $request): Response
	{
		...
		return Inertia::render('Works/Browse', ['prev_url' => URL::previous(), 'items' => $items, 'default_filters' => $this->defaultFilters(), 'filters' => ['date' => $date, 'patient' => $patient, 'mechanics' => ['selected' => $mid, 'items' => Mechanic::all()
																																																															 ->map(fn($m) => ['value' => $m->id, 'label' => $m->name]),], 'clinics' => ['selected' => $cid, 'items' => Clinic::all(),], 'lock_type' => $lockType], 'sort' => $sort, 'direction' => $direction,]);
	}

	public function create(): Response
	{
		return Inertia::render('Works/Create', ['prev_url' => URL::previous(), 'clinics' => Clinic::all(), 'mechanics' => Mechanic::all(), 'works_types' => WorkType::all()]);
	}
	...
	public function edit(string $id): Response
	{
		...
		$currentWorkTypes = $work->workTypes->map(function ($type)
		{
			return ['id' => $type->id, 'quantity' => $type->pivot->count];
		})->toArray();

		return Inertia::render('Works/Update', ['prev_url' => URL::previous(), 'data' => ['item' => $work, 'states' => [self::STATE_NOT_START => 'Не начато', self::STATE_IN_PROCESS => 'В процессе', self::STATE_COMPLETED => 'Завершено', self::STATE_SENT => 'Отправлено',], 'clinics' => Clinic::all(), 'mechanics' => Mechanic::all(), 'works_types' => WorkType::all(), 'current_work_types' => $currentWorkTypes, // передаём текущие типы
		]]);
	}
	...
	public function update(WorksRequest $request, string $id): RedirectResponse
	{
		...
		$work->update(['start' => $validated['start'], 'end' => $validated['end'], 'state' => $validated['state'], 'patient' => $validated['patient'], 'cid' => $validated['cid'], 'mid' => $validated['mid'], 'comment' => $validated['comment'] ?? null,]);

		$pivotData = [];
		foreach ($validated['works'] as $workType)
		{
			$pivotData[$workType['id']] = ['count' => $workType['quantity']];
		}
		...
		if ($validated['state'] == self::STATE_COMPLETED)
		{
			...
			$work->update(['cost_clinic' => $totalClinic, 'cost_mechanic' => $totalMechanic,]);
		}
